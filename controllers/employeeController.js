  ...
  try {
    ...
    if (user) {
      return res
        ...
        .json({
          success: false,
          error: "User Already Registered in Employees List",
        });
    }
    ...
    return res
      .status(500)
      .json({ status: false, error: "Server Error in Add Employee" });
  }
};
...
const fetchEmployeesByDepartmentID = async(req, res) => {
  const {id} = req.params;
  try {
    const employees = await Employee.find({department: id})
    return res.status(200).json({success: true, employees})
  } catch(error) {
    return res.status(500).json({success: false, error: "Fetch Employees By Department ID Server Error"})
  }
}

export { addEmployee, upload, getEmployees, getEmployee, updateEmployee, fetchEmployeesByDepartmentID };
